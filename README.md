...
* [ ] 用户通过邮箱密码注册（密码须加密存储bcrypt或同等方案）
* [ ] 用户登录后返回JWT
* [ ] 注册时或由管理员分配角色（角色：上传者、观看者、管理员）
* [ ] 受保护路由（所有上传/管理接口须携带有效Token）

2. 视频上传

* [ ] 将视频文件上传至阿里云OSS（支持 MP4、MOV、AVI、MKV格式）
* [ ] 大文件分片/分块上传（文件可达TB级，不可接受单流上传，WebWorker）
* [ ] 断点续传 — 上传失败或中断后可恢复（通过服务端或OSS分片API追踪分片状态）
* [ ] 向客户端返回上传进度（显示百分比或已上传字节数）
* [ ] 分片失败自动重试（可配置重试次数，每个分片至少重试3次，采用指数退避策略）
* [ ] 上传前进行文件大小和类型校验（尽早拒绝不支持的格式）
* [ ] 并发上传 — 同时上传多个文件（适当排队和限流）
* [ ] 取消上传（中止分片上传并清理OSS 中的临时数据）


3. 存储与文件管理

* [ ] 在数据库中存储视频元数据（标题、上传者、大小、状态、OSS Key）
* [ ] 软删除 — 将视频标记为已删除，不立即从OSS移除（通过定时任务或管理员操作执行永久删除）
* [ ] 按用户统计存储使用量（可用于配额管理）
